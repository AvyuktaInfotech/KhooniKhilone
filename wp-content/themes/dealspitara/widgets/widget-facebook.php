...
add_action( 'widgets_init', 'maxmag_facebook_load_widgets' );

function maxmag_facebook_load_widgets() {
	register_widget( 'maxmag_facebook_widget' );
}

class maxmag_facebook_widget extends WP_Widget {

	/**
	 * Widget setup.
	 */
	function maxmag_facebook_widget() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'maxmag_facebook_widget', 'description' => __('A widget that displays a Facebook Like Box.', 'maxmag_facebook_widget') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'maxmag_facebook_widget' );

		/* Create the widget. */
		$this->WP_Widget( 'maxmag_facebook_widget', __('Max Mag: Facebook Widget', 'maxmag_facebook_widget'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<iframe src="//www.facebook.com/plugins/likebox.php?href=<?php echo $page_url; ?>&amp;width=<?php echo $width; ?>&amp;height=258&amp;show_faces=<?php if($faces) { echo 'true'; } else { echo 'false'; } ?>&amp;colorscheme=light&amp;stream=false&amp;border_color=%23ffffff&amp;header=<?php if($header) { echo 'true'; } else { echo 'false'; } ?>" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:<?php echo $width; ?>; height:258px;" allowTransparency="true"></iframe>
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => __('Facebook'), 'width' => 300, 'height' => 260, 'faces' => 'on');
